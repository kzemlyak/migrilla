  ...
  async function down() {
    try {
      ...
      const migration = migrations.find(m => {
        const migrationName = `${m.number.toString().padStart(3, '0')}_${m.name}`;
        return migrationName === lastAppliedMigration;
      });

      if (!migration) {
        console.error(`❌ Migration file not found for: ${lastAppliedMigration}`);
        return;
      }

      if (!migration.down) {
        console.error(`❌ Missing down migration file for: ${lastAppliedMigration}`);
        return;
      }

      console.log(`🔄 Rolling back migration: ${lastAppliedMigration}`);
      await executeInTransaction(async (client) => {
        await runMigration(client, migration.down, lastAppliedMigration);
        await markAsReverted(client, lastAppliedMigration);
      });
